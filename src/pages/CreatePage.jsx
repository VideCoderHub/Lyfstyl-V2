  ...
  async function handleSubmit(event) {
    ...
    try {
      if (type === 'recipe') {
        ...
        else if (communityParam) navigate(`/community/${communityParam}`)
        else navigate(`/recipes/${data.recipe.id}`)
      } else {
        ...
        else navigate(`/moves/${data.move.id}`)
      }
    } catch (err) {
      ...
      <PageHero
        ...
        actions={
          challenge ? (
            ...
            <Link to={`/community/${communityInfo.slug}`} className="btn btn--outline">
              Back to community
            </Link>
          ) : null
        }
